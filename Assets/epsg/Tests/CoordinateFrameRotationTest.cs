...
public class CoordinateFrameRotationTest
{
    ...
    public void Testfile1()
    {
        ...
        TextAsset mytxtData = (TextAsset)Resources.Load("GIGS_tfm_5204_CoordFrame_output_part1");
        ...
        for (int i = 0; i < linecount; i++)
        {
            ...
            double lat2;
            double.TryParse(lineparts[3], out lat2);
            double lon2;
            double.TryParse(lineparts[4], out lon2);
            
            if (lineparts[6] == "REVERSE")
            {
                bool withinTolerance = Test1(lat2,lon2,CoordinateReferenceSystem.WGS84,lat,lon);
            ...
            if (lineparts[6] == "FORWARD")
            {
                ...
                bool withinTolerance = Test1(lat, lon, CoordinateReferenceSystem.Amersfoort, lat2, lon2);
    ...
    public void Testfile2()
    {
        ...
        for (int i = 0; i < linecount; i++)
        {
            ...
            if (lineparts[6] == "REVERSE")
            {
                bool withinTolerance = Test2(lat2, lon2,height2, CoordinateReferenceSystem.WGS84, lat, lon, height);
            ...
            if (lineparts[6] == "FORWARD")
            {
                ...
                bool withinTolerance = Test2(lat, lon,height, CoordinateReferenceSystem.Amersfoort, lat2, lon2,height2);
    ...
    public bool Test1(double Lat, double Lon,CoordinateReferenceSystem sourceCRS,double targetLat, double targetLon)
    {
        CRS WGScrs = new WGS84();
        CRS crs = new Amersfoort();
        crs.toWGS84 = new double[] { -106.8686, 52.2978, -103.7239, -0.3366, 0.457, -1.8422, -1.2747 };
        crs.ellipsoid = new Ellipsoid(6378388, 297);

        Vector3LatLong target = new Vector3LatLong(targetLat, targetLon, 0);

        if (sourceCRS == CoordinateReferenceSystem.WGS84)
        {
            
            Vector3GeoCentric sourceGeocentric = WGScrs.ToGeoCentric(new Vector3LatLong(Lat, Lon, 80d));
            Vector3GeoCentric targetGeocentric = crs.FromWGS84GeoCentric(sourceGeocentric);
            Vector3LatLong result = crs.ToEllipsoidal(targetGeocentric);
            return CheckLatLong(result, target, 0.0000003);
        }
       else
        {
            Vector3GeoCentric sourceGeocentric = crs.ToGeoCentric(new Vector3LatLong(Lat, Lon, 80d));
            Vector3GeoCentric targetGeocentric = crs.ToWGS84GeoCentric(sourceGeocentric);
    ...
    public bool Test2(double Lat, double Lon, double height, CoordinateReferenceSystem sourceCRS, double targetLat, double targetLon, double targetHeight)
    {
        CRS WGScrs = new WGS84();
        CRS crs = new Amersfoort();
        crs.toWGS84 = new double[] { -106.8686, 52.2978, -103.7239, -0.3366, 0.457, -1.8422, -1.2747 };
