...
{
    public static class GeographicToGeocentric
    {


        public static Vector3Any Forward(Vector3Any coordinate, GeographicToGeocentricSettings settings)
        {
            double lattitude = coordinate.value1 * Math.PI / 180.0;
            double longitude = (coordinate.value2+settings.primeMeridian) * Math.PI / 180.0;
            ...
            double Z = ((1 - Math.Pow(settings.eccentricity, 2)) * primeVerticalRadius + ellipisoidalHeight) * Math.Sin(lattitude);
            return new Vector3Any(X,Y,Z);

        }
        public static Vector3Any Reverse(Vector3Any coordinate, GeographicToGeocentricSettings settings)
        {
            ...
            return new Vector3Any( lattitude * 180 / Math.PI, (longitude * 180 / Math.PI)-settings.primeMeridian, height);
