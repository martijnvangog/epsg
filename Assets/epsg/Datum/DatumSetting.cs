...
{
    [Serializable]
    public abstract class DatumSetting
    {
        public string name;
        public int epsgCode;
        public Ellipsoid ellipsoid;
        public int connectedDatum;
        public double[] toConnectedDatum;
        ...
        double tZ;
        double M;
        double[,] reverseRotationMatrix;

        public Vector3Geographic ToEllipsoidal(Vector3GeoCentric coordinate)
        {
            ...
            return new Vector3Geographic(lattitude * 180 / Math.PI, longitude * 180 / Math.PI, height);
        }

        public Vector3GeoCentric ToGeoCentric(Vector3Geographic coordinate)
        {
        ...
        public Vector3GeoCentric ToConnectedGeoCentric(Vector3GeoCentric coordinate)
        {
            if (rotationMatrix is null)
            {
                SetupMatrices();
            }
            double X = (double)(coordinate.x * rotationMatrix[0, 0] + coordinate.y * rotationMatrix[0, 1] + coordinate.z * rotationMatrix[0, 2]) * M + tX;
            double Y = (double)(coordinate.x * rotationMatrix[1, 0] + coordinate.y * rotationMatrix[1, 1] + coordinate.z * rotationMatrix[1, 2]) * M + tY;
            double Z = (double)(coordinate.x * rotationMatrix[2, 0] + coordinate.y * rotationMatrix[2, 1] + coordinate.z * rotationMatrix[2, 2]) * M + tZ;
            return new Vector3GeoCentric(X, Y, Z);
        }
        public Vector3GeoCentric FromConnectedGeoCentric(Vector3GeoCentric coordinate)
        {
            if (rotationMatrix is null)
            {
                SetupMatrices();
            ...
            double X = (double)(coordinate.x * reverseRotationMatrix[0, 0] + coordinate.y * reverseRotationMatrix[0, 1] + coordinate.z * reverseRotationMatrix[0, 2]) / M;
            double Y = (double)(coordinate.x * reverseRotationMatrix[1, 0] + coordinate.y * reverseRotationMatrix[1, 1] + coordinate.z * reverseRotationMatrix[1, 2]) / M;
            double Z = (double)(coordinate.x * reverseRotationMatrix[2, 0] + coordinate.y * reverseRotationMatrix[2, 1] + coordinate.z * reverseRotationMatrix[2, 2]) / M;
            return new Vector3GeoCentric(X, Y, Z);
        }
        ...
        void SetupMatrices()
        {
            ...
            tX = toConnectedDatum[0];
            tY = toConnectedDatum[1];
            tZ = toConnectedDatum[2];
            double rX = toConnectedDatum[3] * arsecToRad;
            double rY = toConnectedDatum[4] * arsecToRad;
            double rZ = toConnectedDatum[5] * arsecToRad;
            double dS = toConnectedDatum[6] / Math.Pow(10, 6);
            M = 1 + (double)dS;
            ...
            double det =rotationMatrix[0, 0] * coFactors[0, 0] + rotationMatrix[0, 1] * coFactors[0, 1] + rotationMatrix[0, 2] * coFactors[0, 2];

            reverseRotationMatrix = new double[3, 3];
        ...
        double getDeterminant(int y, int x)
        {
            ...
            return rotationMatrix[toprow, leftcolumn] * rotationMatrix[bottomrow, rightcolumn] - rotationMatrix[toprow, rightcolumn] * rotationMatrix[bottomrow, leftcolumn];
        }
    }
}
